...
你会注意到这个 ABI 定义了一个 `transfer` 类型的动作 `transfer` 。这告诉 EOSIO，当 `${account}->transfer` 被看到时，交易的负载（payload）类型是 `transfer` 。动作类型 `transfer` 在 `structs` 数组中被定义，`structs` 数组对象中，`name` 属性的值为 `transfer`。
